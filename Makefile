...
# Example for profiling run (uses one source; adjust if needed)
...
# Default: release build without PGO (fast to build, good for development)
all: release

release: $(SRC)
	$(CXX) $(CXXFLAGS) -flto -o $(TARGET) $(SRC)

# Full PGO build: instrument → profile → optimise
pgo: $(SRC)
	@echo "=== Step 1: build instrumented binary ==="
	$(CXX) $(CXXFLAGS) -fprofile-instr-generate -o $(PROF_EXE) $(SRC)
	...
	$(CXX) $(CXXFLAGS) -flto -fprofile-instr-use=$(PROF_DATA) -o $(TARGET) $(SRC)
	@echo "=== Done: $(TARGET) ==="
